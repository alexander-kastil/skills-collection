...
description: "Use Xquik REST or MCP to retrieve, monitor, and export public X data for agent workflows."
...
Use this skill to plan and execute read-oriented workflows with Xquik, a real-time X (Twitter) data platform with REST, MCP, extraction, monitoring, and webhook surfaces. The skill helps agents choose between REST and MCP, discover the right endpoint from the public specification, handle authentication safely, and normalize public X data for downstream analysis.
...
- Use when the user asks an agent to collect, search, monitor, export, or analyze public X data through Xquik.
...
- **Writes:** Keep this skill read-oriented. Route write tasks through explicit user confirmation and the current product docs.
...
- Start or inspect extraction jobs.
- Create account or keyword monitors.
- Read events for webhook-style processing.

If the task can be answered with existing local data, use that first.

### 2. Choose REST or MCP

Use REST when the user needs deterministic programmatic calls, backend integration, export jobs, pagination control, or HMAC webhook handling.
...
- Result count.
- Next cursor or extraction ID when present.
...
### Example 3: Start a Tweet Search Extraction

```bash
curl "https://xquik.com/api/v1/extractions" \
  -H "content-type: application/json" \
  -H "x-api-key: $XQUIK_API_KEY" \
  -d '{"toolType":"tweet_search_extractor","searchQuery":"from:nvidia AI","resultsLimit":100}'
```

## Best Practices

- Verify endpoint names and parameters against the OpenAPI spec before calling.
- Keep workflows read-only unless the user explicitly asks for a write action.
