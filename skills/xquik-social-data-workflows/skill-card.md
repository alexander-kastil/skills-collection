...
Xquik Social Data Workflows helps agents use Xquik REST or MCP to retrieve and inspect public X data for agent workflows.
...
Use this skill when an AI agent, backend service, or workflow automation needs authenticated access to Xquik REST or MCP for public X data retrieval, existing extraction or monitor inspection, and analysis.
...
Risk: Accidental resource creation or write actions from a read workflow.

Mitigation: Keep this skill read-only. Move resource creation and write actions to a separate, correctly risk-classified workflow with explicit user confirmation.
...
The skill produces endpoint plans, safe REST or MCP read patterns, normalized result summaries, and follow-up instructions for pagination, retries, existing extraction results, monitors, or event processing.
